...
import it.gov.pagopa.apiconfig.datamigration.repository.oracle.IbanValidiPerPaSrcRepository;
import it.gov.pagopa.apiconfig.datamigration.repository.postgres.IbanValidiPerPaDestRepository;
...
@Slf4j
@Service("EXECUTE_IBAN_VALIDI_PER_PA_TABLE_MIGRATION")
public class ExecuteIBANValidiPerPATableMigrationStep extends Step {

    private static final int PAGE_SIZE = 50;

    @Autowired
    IbanValidiPerPaSrcRepository srcRepo;

    @Autowired
    IbanValidiPerPaDestRepository destRepo;
    ...
    public void executeStep() throws MigrationStepException {
        try {
            ...
            do {
                Page<IbanValidiPerPa> pagedEntities = srcRepo.findAll(pageable);
                List<IbanValidiPerPa> entities = pagedEntities.getContent();
    ...
    public String getStepName() {
        return StepName.EXECUTE_IBAN_VALIDI_PER_PA_TABLE_MIGRATION.toString();
    }

    @Override
    public DataMigrationStatus getDataMigrationStatus(DataMigrationDetails details) {
        return details.getIbanValidiPerPa();
    }
}
