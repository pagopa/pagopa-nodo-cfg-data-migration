...
@SpringBootTest(classes = ExecuteBinaryFileTableMigrationStep.class)
class ExecuteBinaryFileTableMigrationStepTest {
    ...
    @Autowired
    @Spy
    ExecuteBinaryFileTableMigrationStep migrationStep;
    ...
    void testExecuteStep() throws MigrationStepException, IllegalAccessException, NoSuchFieldException {
        ...
        Page<BinaryFile> pagedResponse = new PageImpl<>(Collections.singletonList(binaryFile));

        // Emulating findAll
        Field field = ExecuteBinaryFileTableMigrationStep.class.getDeclaredField("PAGE_SIZE");
    ...
    void getNextState() {
        ...
        assert nextState == StepName.EXECUTE_INFORMATIVE_CONTO_ACCREDITO_MASTER_TABLE_MIGRATION;
    }

    @Test
    void getStepName() {
        String stepName = migrationStep.getStepName();
        assert stepName.equals("EXECUTE_BINARY_FILE_TABLE_MIGRATION");
    }
}
